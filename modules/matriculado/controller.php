	...
	function new_matriculas($arg) {
		...
		if (is_array($matricula_collection) AND !empty($matricula_collection)) {
			foreach ($matricula_collection as $clave=>$valor) {
				...
				// COMPRUEBO POR VIEJO MÉTODO DE CTA CTE
				...
				$group_by = "ccm.anio";
				$ctactemat = CollectorCondition()->get('CuentaCorrienteMatriculado', $where, 4, $from, $select, $group_by);
				$ctactemat = (is_array($ctactemat) AND !empty($ctactemat)) ? $ctactemat[0]["CANT"] : 0;
				...
				if ($ctactemat > 0) {
					...
					$ctactemat_venc = CollectorCondition()->get('CuentaCorrienteMatriculado', $where, 4, $from, $select, $group_by);
					$ctactemat_venc = (is_array($ctactemat_venc) AND !empty($ctactemat_venc)) ? $ctactemat_venc[0]["FECHA"] : 0;
					if ($ctactemat_venc == 0) {
						$btn_arg_estado = 0;
						...
						$flag_matricula_activa = 1; 
					} else {
						if ($ctactemat_venc > $fecha) {
							$btn_arg_estado = 0;
							$btn_class_estado = 'danger';
							$btn_icon_estado = 'arrow-down';
							$lbl_estado = 'ACTIVA';
				...
				// COMPRUEBO POR NUEVO MÉTODO DE PAGO DIRECTO
				if ($flag_matricula_activa == 0) {
		...
		if (is_array($matricula_collection) AND !empty($matricula_collection)) {
			foreach ($matricula_collection as $clave=>$valor) {
				...
				if (!empty($estado_collection)) {
					...
					if ($estado == 'ACTIVA' AND $perfil == 'admin') {
						$btn_arg_estado = 0;
						$btn_class_estado = 'danger';
						$btn_icon_estado = 'arrow-down';
						$lbl_estado = 'ACTIVA';
